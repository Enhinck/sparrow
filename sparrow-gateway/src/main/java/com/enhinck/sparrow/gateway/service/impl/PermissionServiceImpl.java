		...
		if (principal != null) {
			if (CollectionUtil.isEmpty(grantedAuthorityList)) {
				if (log.isDebugEnabled()) {
					log.debug("角色列表为空：{}", authentication.getPrincipal());
				}
			...
			for (GrantedAuthority grantedAuthority : grantedAuthorityList) {
				...
				RoleVO roleVO = roleCache.get(roleCode);
				if (roleVO != null && CollectionUtils.isNotEmpty(roleVO.getPermissionVOS())) {
					List<PermissionVO> permissionVOs = roleVO.getPermissionVOS();
			...
			if (CollectionUtils.isNotEmpty(permissionIds)) {
				RoleVO roleVO = roleCache.get("ROLE_USER");
				if (roleVO != null && CollectionUtils.isNotEmpty(roleVO.getPermissionVOS())) {
					List<PermissionVO> permissionVOs = roleVO.getPermissionVOS();
					for (PermissionVO permissionVO : permissionVOs) {
						permissionIds.add(permissionVO.getId());
			...
			Set<MenuVO> menuSet = menuCache.getAllSet();
			// 权限过滤
			Set<MenuVO> urls = SysUtil.filter(menuSet, permissionIds);
